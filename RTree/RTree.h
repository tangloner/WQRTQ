...
// a_rect是待插入的rectangle, a_id是rectang所属的DATATYPE, a_node是等待插入树的根节点, a_level是待插入的层数
// 本层分裂用new_node, 下层分裂用other_node
RTREE_TEMPLATE
bool RTREE_QUAL::InsertRectRec(Rect* a_rect, const DATATYPE& a_id, Node* a_node, Node** a_newNode, int a_level)
{
    ...
    if(a_node->m_level > a_level)
    {
        ...
        // 下层有分裂, a_node->m_branch[index].m_child已经改变
...
// a_minFill 最小填充几个数值, m_total所有的branch数量
RTREE_TEMPLATE
void RTREE_QUAL::ChoosePartition(PartitionVars* a_parVars, int a_minFill)
{
    ...
    // m_total-m_minFill代表不能均分时较大的分支可含有的entry数量, 例如M=8, m=4, 则实际分裂时会出现4 5两种子树节点数量
    // 所以当某个分支到达5时, 为了保证符合分裂条件, 不能够向5的节点继续添加entry
    while (((a_parVars->m_count[0] + a_parVars->m_count[1]) < a_parVars->m_total)
           && (a_parVars->m_count[0] < (a_parVars->m_total - a_parVars->m_minFill))
           && (a_parVars->m_count[1] < (a_parVars->m_total - a_parVars->m_minFill)))
    {
        biggestDiff = (ELEMTYPEREAL) -1; //并入和没并入相比不会减少面积
...
void RTREE_QUAL::PickSeeds(PartitionVars* a_parVars)
{
    ...
    // 任意两个区域内的rectangle, 最大不超过整个rectangle, 计算waste最大不超过整个rectangle的面积, 再减1
    worst = -a_parVars->m_coverSplitArea - 1;
    // 任意branch中的area组合做浪费空间对比
